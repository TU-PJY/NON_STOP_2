...
class Commando:
    ...
    def __init__(self):
        ...
        self.x, self.y, self.Dir = WIDTH / 2, 150, 1
        self.mv_right, self.mv_left, self.mv_jump = False, False, False
        self.jump_acc = JUMP_ACC

    def draw(self):
        if self.Dir == 1: self.image.clip_draw(0, 0, 100, 100, self.x, self.y, 70, 70)
        elif self.Dir == 0: self.image.clip_composite_draw(0, 0, 100, 100, 0, 'h', self.x, self.y, 70, 70)
           
    def move(self):#좌우 이동 
        if self.mv_right == True: self.x += 4
        if self.mv_left == True: self.x -= 4
