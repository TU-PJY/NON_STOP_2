...
class Gun:
    ...
    def draw(self):
        if GUN_NAME == 'SCAR_H':  # GUN_NAME에 따라 사용하는 총기가 달라진다.
            if self.p.dir == 1:
                self.scar_right.clip_composite_draw(0, 0, 150, 100, self.deg, '', self.p.x, self.p.y - self.p.land_y, 170, 120)
            elif self.p.dir == 0:
                self.scar_left.clip_composite_draw(0, 0, 150, 100, self.deg, 'h, v', self.p.x, self.p.y - self.p.land_y, 170, 120)
